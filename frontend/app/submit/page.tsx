...
import { ArrowLeft, MapPin, Camera, X, Clock, Phone, Globe, Utensils, AlertCircle, CheckCircle } from "lucide-react"
...
export default function SubmitSpotPage() {
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      const newImages = [...form.images, ...files].slice(0, 5) // Max 5 images
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      case 1:
        ...
        if (form.description.length < 50) newErrors.description = "Description must be at least 50 characters"
        break
      case 2:
        if (!form.priceRange) newErrors.priceRange = "Price range is required"
        if (form.cuisine.length === 0) newErrors.cuisine = "Select at least one cuisine type"
        break
      case 3:
    ...
    try {
      ...
      const verificationResponse = await fetch("/api/ai-verify", {
        ...
        body: JSON.stringify({
          ...
          images: form.imageUrls, // Use the URLs for now
        }),
      })
      ...
      router.push(`/?submitted=true&status=${verificationData.verification.status}`)
    } catch (error) {
    ...
    switch (currentStep) {
      case 1:
        return (
          <div className="space-y-6">
            <div>
              <h2 className="text-subheading font-bold text-foreground mb-2">Basic Information</h2>
              <p className="text-body text-muted-foreground">Tell us about this Amala spot</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Textarea
                  ...
                  rows={4}
                  className={cn("mt-1 resize-none", errors.description && "border-destructive")}
                />
      ...
      case 2:
        return (
          <div className="space-y-6">
            <div>
              <h2 className="text-subheading font-bold text-foreground mb-2">Cuisine & Pricing</h2>
              <p className="text-body text-muted-foreground">Help people find the right spot</p>
            </div>

            <div className="space-y-6">
              <div>
                <Label className="text-body font-medium">Price Range *</Label>
                <Select value={form.priceRange} onValueChange={(value) => handleInputChange("priceRange", value)}>
                  <SelectTrigger className={cn("mt-1", errors.priceRange && "border-destructive")}>
                    <SelectValue placeholder="Select price range" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="$">$ - Budget friendly (Under $15)</SelectItem>
              ...
              <div>
                <Label className="text-body font-medium">Cuisine Types *</Label>
              ...
              <div>
                <Label className="text-body font-medium">Amenities</Label>
                <p className="text-caption text-muted-foreground mb-3">What does this place offer?</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
      ...
      case 3:
        return (
          <div className="space-y-6">
            <div>
              <h2 className="text-subheading font-bold text-foreground mb-2">Contact & Hours</h2>
              <p className="text-body text-muted-foreground">Help customers reach and visit</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-caption text-muted-foreground mb-4">Set the hours for each day</p>
      ...
      case 4:
        return (
          <div className="space-y-6">
            <div>
              <h2 className="text-subheading font-bold text-foreground mb-2">Photos</h2>
              <p className="text-body text-muted-foreground">Show off this amazing spot (max 5 photos)</p>
            </div>
            ...
            <div className="space-y-4">
              <div
                className={cn(
                  ...
                  errors.images ? "border-destructive" : "border-border hover:border-primary",
                )}
              >
                <input
                  type="file"
                ...
                <label
                  ...
                  className={cn("cursor-pointer", form.images.length >= 5 && "cursor-not-allowed opacity-50")}
                >
                  <Camera className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                  <p className="text-body font-medium text-foreground mb-2">
                    {form.images.length >= 5 ? "Maximum photos reached" : "Upload Photos"}
                  </p>
                  <p className="text-caption text-muted-foreground">
                    Drag and drop or click to select ({form.images.length}/5)
                  </p>
                </label>
              </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-subheading font-bold text-foreground">Add Amala Spot</h1>
                <p className="text-caption hidden sm:block">Share a great spot with the community</p>
              </div>
            </div>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {isVerifying && (
              <div className="mb-6 p-4 bg-primary/10 border border-primary/20 rounded-lg">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-primary">AI Verification in Progress</h3>
                    <p className="text-sm text-muted-foreground">
                      Our AI is analyzing your submission for authenticity and accuracy...
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6 p-4 border rounded-lg">
                <div className="flex items-center space-x-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Confidence Score: {verificationResult.score}%</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <ul className="space-y-1">
                    {verificationResult.reasons?.slice(0, 3).map((reason: string, index: number) => (
        ...
        <div className="flex items-center justify-between mt-8">
          ...
          <div className="flex space-x-3">
            {currentStep < totalSteps ? (
              <Button onClick={nextStep} disabled={isSubmitting} className="bg-primary hover:bg-primary/90">
                Next Step
              </Button>
            ) : (
        ...
        <Card className="mt-8 bg-muted/50">
          <CardContent className="p-6">
            <h3 className="text-subheading font-bold text-foreground mb-3">Submission Guidelines</h3>
